...
import {
  useListCustomerLoans, getListCustomerLoansQueryKey, useCreateCustomerLoan,
  useCorrectCustomerLoan, useListCustomers, getListCustomersQueryKey,
  type CustomerLoan,
...
const defaultForm = (): CustomerLoanForm => ({
  customerId: "",
...
export default function CustomerLoansPage() {
  ...
  const [form, setForm] = useState<CustomerLoanForm>(defaultForm());
  ...
  const { data: loans = [], isLoading } = useListCustomerLoans(params, { query: { queryKey: getListCustomerLoansQueryKey(params) } });
  const { data: customers = [] } = useListCustomers(undefined, { query: { queryKey: getListCustomersQueryKey() } });
  const createMutation = useCreateCustomerLoan();
  const correctMutation = useCorrectCustomerLoan();
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getListCustomerLoansQueryKey() });
      queryClient.invalidateQueries({ queryKey: getListCustomersQueryKey() });
      setShowForm(false);
      setForm(defaultForm());
      toast({ title: "Loan recorded" });
    } catch {
      toast({ title: "Failed to record loan", variant: "destructive" });
    }
  };

  // ── Correction (reverse + optionally replace) ──
  const [correcting, setCorrecting] = useState<CustomerLoan | null>(null);
  const [correctForm, setCorrectForm] = useState<CustomerLoanForm>(defaultForm());
  const [correctVoid, setCorrectVoid] = useState(false);
  const [correctReason, setCorrectReason] = useState("");

  const openCorrect = (p: CustomerLoan) => {
    setCorrecting(p);
    setCorrectForm({
      customerId: p.customerId, date: p.date, paymentMode: p.paymentMode as PaymentMode, amount: String(p.amount),
  ...
  const handleCorrect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: getListCustomersQueryKey() });
      setCorrecting(null);
      toast({ title: correctVoid ? "Loan voided" : "Loan corrected" });
    } catch {
      toast({ title: "Failed to correct loan", variant: "destructive" });
    }
  };

  const totalLent = loans.filter(p => p.status === "posted").reduce((s, p) => s + p.amount, 0);

  // One row per logical transaction: `head` is its current effective state.
  const groups = groupCorrections(loans);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            Total lent: Rs. {formatAmount(totalLent)}
          </p>
        </div>
        <Button onClick={openNewForm}><Plus size={15} className="mr-1.5" /> Lend Money</Button>
      </div>
      ...
      <div className="bg-card border border-card-border rounded-xl p-4 mb-4 flex flex-wrap gap-3 items-end">
        ...
        <Button variant="outline" size="sm" onClick={() => { setFromDate(""); setToDate(""); setFilterCustomerId(undefined); setFilterMode(undefined); }}>Clear</Button>
      </div>
      ...
      <div className="bg-card border border-card-border rounded-xl shadow-xs overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            ...
            {!isLoading && groups.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">No customer loans found</td></tr>}
            {groups.map(g => {
              const p = g.head;
              ...
              <Fragment key={p.id}>
                <tr className={cn("hover:bg-muted/20 transition-colors", g.isVoid && "opacity-50")}>
                  ...
                  <td className="px-4 py-3">
                    {!g.isVoid && (
                      <button onClick={() => openCorrect(p)} title="Correct this loan" className="p-1.5 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded transition-colors">
                        <Pencil size={14} />
                      </button>
                    )}
                  </td>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent>
          <DialogHeader><DialogTitle>Lend Money to Customer</DialogTitle></DialogHeader>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button type="submit" className="flex-1" disabled={createMutation.isPending}>
                {createMutation.isPending ? "Recording..." : "Lend Money"}
              </Button>
            </div>
          </form>
        </DialogContent>
      ...
      {/* Correct Loan Dialog — reverses the original behind the scenes, never edits it */}
      <Dialog open={!!correcting} onOpenChange={(o) => { if (!o) setCorrecting(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Undo2 size={16} /> Correct Loan #{correcting?.id}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleCorrect} className="space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button type="submit" className="flex-1" disabled={correctMutation.isPending}>
                {correctMutation.isPending ? "Saving..." : correctVoid ? "Void Loan" : "Save Correction"}
              </Button>
            </div>
          </form>
        </DialogContent>
