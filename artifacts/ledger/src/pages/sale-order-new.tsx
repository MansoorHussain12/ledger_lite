...
export default function SaleOrderNewPage() {
  ...
  const [items, setItems] = useState<LineItem[]>([{ productId: 0, productName: "", qty: "", rate: "" }]);
  ...
  const handleProductChange = (idx: number, productId: number) => {
    ...
    setItems(prev => prev.map((item, i) =>
      i === idx ? { ...item, productId, productName: product?.name ?? "", rate: product ? String(product.currentRate) : "" } : item
    ));
  };

  const addLine = () => setItems(prev => [...prev, { productId: 0, productName: "", qty: "", rate: "" }]);
