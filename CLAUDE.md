...
- `lib/db` — Drizzle schema and DB client. Schema files live in `lib/db/src/schema/`: `customers`, `products` (+ `product_rates`), `saleOrders` (`sale_orders`, `sale_order_items`), `payments`, `customerLoans` (cash lent to a customer, no interest — increases receivable balance like a sale), `cashbook` (`cashbook_entries`, `expenses`), `purchases` (`suppliers`, `purchase_invoices`, `purchase_invoice_items`), `supplierPayments`, `installments` (EMI plans/schedule/payments), `inventory` (`stock_adjustments`), `lookups` (generic key/value config), `users`. `index.ts` re-exports all tables.
...
- Cash purchases/payments auto-post to `cashbook_entries`; deleting a purchase removes the matching cashbook entry. Customer loans (`customer_loans`) auto-post the same way, as a `cash_out`/`customer_loan` entry — correcting/voiding a loan reverses its cashbook entry too.
- Supplier balance = `opening_balance + sum(purchase total_amount) − sum(paid_amount)`. Customer balance = `opening_balance + sum(sale total_amount) − sum(payment amount) + sum(customer_loans amount)` (a loan is a debit, same direction as a sale — no interest, just tracked as receivable until repaid via a normal payment).
- Company settings (`company_settings`) is a single-row table accessed via raw pool queries (not the Drizzle query builder elsewhere): GET is public, PUT is owner-only, logo is stored as base64 `TEXT`. It is **not** part of the Drizzle schema (not in `lib/db/src/schema/`) and `pnpm run setup:local`/`push-force` will not create it — `scripts/seed.sql` has a `CREATE TABLE IF NOT EXISTS` for it, so run that at least once against any DB you want the app's settings page to actually work on. It (and `user_sessions`, also unmanaged by Drizzle) is protected from `drizzle-kit push` via `tablesFilter` in `drizzle.config.ts` — see the Gotcha below for why that exists.
...
## Gotchas
- Always ask before pushing or commiting any change on git and github.
...
- **`drizzle-kit push`/`push-force` will silently DROP any table that exists in the DB but has no corresponding Drizzle schema file** — this actually happened during development: running `push-force` to add `audit_logs` also dropped the unrelated, manually-created `company_settings` table, since drizzle-kit treats "table in DB but not in schema" as drift to reconcile away. Fixed via `tablesFilter: ["!company_settings", "!user_sessions"]` in `lib/db/drizzle.config.ts`, which excludes both non-Drizzle-managed tables from the diff entirely (uses `drizzle-kit`'s Minimatch-based glob filter, which supports `!`-negation). **If you ever add another raw/non-Drizzle table, add it to that `tablesFilter` list too, or it will get dropped the next time anyone runs `push`/`push-force` — including against the production Neon DB.** One harmless side effect remains: push output may still print a JS error about failing to drop `company_settings_id_seq` (the backing sequence for its `id` column) — that DROP SEQUENCE statement isn't table-filtered, but it always fails safely because Postgres won't drop a sequence that's still a live column default, which it structurally always is as long as the table itself survives. That printed error is expected and not a problem.
